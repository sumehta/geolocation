...
if not opt then
   ...
   cmd:option('-input_onehot_h5','data/train/country_one_hot.h5','path to one hot vectors for countries')
   cmd:option('-save', 'results/town/', 'subdirectory to save/log experiments in')
...
local h5_onehot_file = hdf5.open(opt.input_onehot_h5, 'r')

local data = h5_img_file:read('/image-features'):all()
local labels = h5_label_file:read('/town_labels'):all()
local country_one_hot = h5_onehot_file:read('/country_one_hot'):all()

h5_img_file:close()
h5_label_file:close()
h5_onehot_file:close()
    ...
    for t = 1,trsize,opt.batchSize do
      -- create mini batch
          ...
          for i = t,math.min(t+opt.batchSize-1,trsize) do
             ...
             -- load country one-hot vectors
             local one_hots = country_one_hot[shuffle[i]]
             one_hots = one_hots:double()
             -- concatenate the inputs with one-hot vecotors
             input = torch.cat(input, one_hots)
